...
contract TransitStationIntegrationTest is Test {
    ...
    function setUp() public {
        ...
        routes[3] =
            TransitStation.Route({ destEID: DST_EID, offerAsset: address(offerToken), wantAsset: address(wantToken) });
    ...
    function _simulateLzReceive(TransitStation.OrderTerms memory terms) internal {
        ...
        Origin memory origin =
            Origin({ srcEid: uint32(endpoint.eid()), sender: bytes32(uint256(uint160(address(station)))), nonce: 0 });
        ...
        returns (bytes32 uuid, TransitStation.OrderTerms memory terms)
    {
        ...
        bytes32 eventSig = keccak256("OrderBridged(bytes32,uint32,bytes32,(bytes32,address,address,address,uint256))");
