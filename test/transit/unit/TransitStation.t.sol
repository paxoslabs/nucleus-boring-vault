...
import { MockEndpoint } from "test/transit/mocks/MockEndpoint.sol";
...
contract TransitStationTest is Test {
    ...
    function _deployDefaultStation() internal returns (TransitStation station) {
        station = _deploy(owner, protocolFeeRecipient, quoteSigner, offerReceiver, wantAssetSource, address(endpoint));
    ...
    function _deployStationWithAssets(ERC20 _offerAsset, ERC20 _wantAsset) internal returns (TransitStation station) {
        station = _deploy(owner, protocolFeeRecipient, quoteSigner, offerReceiver, wantAssetSource, address(endpoint));
    ...
    function _deployDefaultStationWithCrossChainRoute() internal returns (TransitStation station) {
        ...
        TransitStation.Route[] memory routes = new TransitStation.Route[](1);
        routes[0] =
            TransitStation.Route({ destEID: DEST_EID, offerAsset: address(offerAsset), wantAsset: address(wantAsset) });
        bool[] memory approved = new bool[](1);
        approved[0] = true;
    ...
    ///      - `offerAmount` is `DEFAULT_OFFER_AMOUNT` (100e6 token units for the 6-decimal offer asset), large
    /// enough
    ...
    function _signQuote(TransitStation station, TransitStation.Quote memory quote)
        internal
        view
    ...
    function testSubmitOrder_RevertIf_IntegratorFeeTooHigh() external {
        ...
        vm.prank(user);
        vm.expectRevert(
            abi.encodeWithSelector(TransitStation.IntegratorFeeTooHigh.selector, maxIntegratorFee + 1, maxIntegratorFee)
        );
        station.submitOrder(quote, signature);
    }
    ...
    function testSubmitOrder_MovesTokensCorrectly() external {
        ...
        assertEq(offerAsset.balanceOf(offerReceiver), offerReceiverBalanceBefore + (DEFAULT_OFFER_AMOUNT - 0.5e6 - 2e6));
    ...
    function testSubmitOrderWithPermit_PermitGrantsAllowanceWhenNoneExists() external {
        ...
        (uint8 v, bytes32 r, bytes32 s) = _getPermitSignature(
            offerPermit, permitUserKey, address(station), DEFAULT_OFFER_AMOUNT_NORMALIZED, deadline
        );
    ...
    function testExecutePendingOrders_RevertIf_WantAssetMismatch() external {
        ...
        batches[0] = TransitStation.FillBatch({ wantAsset: address(otherWantAsset), uuids: uuids, amounts: amounts });
    ...
    /// @dev Mirrors the intended use — an order stuck in bridging is manually refunded, then blocked from
    /// landing.
    ...
    function testGetPendingOrderIds_ReturnsCorrectIds() external {
        ...
        for (uint256 i; i < 3; ++i) {
            ...
            bytes memory signature = _signQuote(station, quote);
            expected[i] = keccak256(abi.encodePacked(hex"1901", _domainSeparator(station), station.hashQuote(quote)));
            vm.prank(user);
            station.submitOrder{ value: 0 }(quote, signature);
        }
    ...
    function testSubmitOrder_EmitsOrderReceived() external {
        ...
        TransitStation.Order memory expectedOrder = TransitStation.Order({
            terms: expectedTerms, amountDue: DEFAULT_OFFER_AMOUNT, queuedAt: uint64(block.timestamp)
        });
    ...
    function testLzReceive_EmitsOrderBridgeReceived() external {
        ...
        TransitStation.Order memory expectedOrder =
            TransitStation.Order({ terms: terms, amountDue: DEFAULT_OFFER_AMOUNT, queuedAt: uint64(block.timestamp) });
