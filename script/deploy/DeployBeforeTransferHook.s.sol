...
contract DeployBeforeTransferHook is BaseScript {
    ...
    address constant REQUIRED_DEPLOYER = 0x1Ab5a40491925cB445fd59e607330046bEac68E5;
    address constant FREEZE_MANAGER = 0x0000000000000000000000000000000000000000;

    /// @param freezeManager Granted `FREEZE_MANAGER_ROLE`, the only role able to call `setFreezeList`. The multisig
    /// can call it regardless, as `Auth` short-circuits for the owner.
    function run(address freezeManager) public broadcast returns (address hook, address rolesAuthority) {
        ...
        require(freezeManager != address(0), "freezeManager must not be zero address");

        bytes32 SALT = makeSalt(
            broadcaster,
            false,
            string("GLOBAL FREEZE LIST BEFORE TRANSFER HOOK")
        );
        ...
        RolesAuthority(rolesAuthority).setUserRole(freezeManager, FREEZE_MANAGER_ROLE, true);
        ...
        require(
            RolesAuthority(rolesAuthority).doesUserHaveRole(freezeManager, FREEZE_MANAGER_ROLE), "freezeManager role"
        );
