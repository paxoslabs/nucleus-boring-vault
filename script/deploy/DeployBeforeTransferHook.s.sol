...
contract DeployBeforeTransferHook is BaseScript {
    ...
    function run() public broadcast returns (address) {
        ...
        address hook = CREATEX.deployCreate3(salt, abi.encodePacked(creationCode, abi.encode(getMultisig())));

        require(FreezeListBeforeTransferHook(hook).owner() == getMultisig(), "owner mismatch");
