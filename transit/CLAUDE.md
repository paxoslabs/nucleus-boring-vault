...
Constructor: `(address _owner, Authority _authority, address _endpoint, address _protocolFeeRecipient, address _quoteSigner, address _offerReceiver)`. Zero-address checks on `_owner`/`_protocolFeeRecipient`/`_quoteSigner`/`_offerReceiver`; code-existence check on `_authority` (interface dep). `thisChainEID` is fetched from `endpoint.eid()` rather than passed in.
...
- `address offerReceiver` — where the net deposit (`offerAmount - protocolFee - integratorFee`) is sent; not held by the station (owner-settable via `setOfferReceiver`). In practice a BoringVault that manages assets, but the contract only ERC20-transfers to it — it doesn't enforce/assume a vault, hence the role-based name.
...
- `setProtocolFeeRecipient(addr) requiresAuth` (zero-checked), `setQuoteSigner(addr) requiresAuth` (zero-checked; no code check — it's an EOA key), `setOfferReceiver(addr) requiresAuth` (zero-checked), `setMessageGasLimit(eid, uint64) requiresAuth`.
...
3. **Custody — split between `offerReceiver` (offer) and station (want); NEEDS RESOLUTION (2026-06-04).** Offer-asset deposits now flow to the configurable `offerReceiver` (in practice a BoringVault) — the station does NOT custody the offer asset (revises KDD 20 for the offer side). BUT `executePendingOrders` still pays the want asset via `safeTransfer` from `address(this)` (the station's own balance, KDD 20). **Inconsistency:** if the BoringVault manages *all* assets, the station holds no want-asset liquidity to `safeTransfer`. Options for the want side: (a) the asset manager pre-funds the station with want-asset liquidity (keeps current code), or (b) `executePendingOrders` pulls the want asset from a configured source via `transferFrom` (source approves the station) — symmetric with offer flowing to `offerReceiver`. **OPEN — awaiting decision.** (Offer side implemented; want side unchanged pending this.)
