...
- `pause() requiresAuth` / `unpause() requiresAuth` — emergency stop via `Pausable` (`src/helper/Pausable.sol`, idempotent `_pause`). **`whenNotPaused` gates `submitOrder`, `submitOrderWithPermit`, AND `executePendingOrders`** (revised 2026-06-08 — pausing now freezes fulfilment too, so a compromised backend can be stopped before funds leave). `_lzReceive` is intentionally NOT gated: no value moves on receive (only on execute, which is gated), and gating it would strand in-flight cross-chain messages. `paused()` is the public view.
...
- *2026-06-08*: **`pause()` now also gates `executePendingOrders`** (added `whenNotPaused`). This is the real compromised-backend kill-switch — submit-only pausing wouldn't stop already-queued malicious orders from being fulfilled (value leaves at execute). `_lzReceive` still ungated (no value moves on receive; gating strands in-flight messages). Comments updated.
