...
- `OrderTerms { bytes32 uuid; address wantAsset; address receiver; uint32 sourceEID; address offerAsset; uint256 offerAmountNormalized18AfterFees; }` — everything fixed at submit time on the source; **this is the LZ wire format** (nothing bridges empty). `offerAmountNormalized18AfterFees` is re-normalized FROM the token units actually collected (never the signed figure). `uuid` is the quote's **EIP-712 digest** (see UUID below).
- `Order { OrderTerms terms; uint256 amountDue; uint64 queuedAt; }` — **composition, not field duplication** (2026-06-10): the source-attested terms plus the two fields only the destination can fill. **`_pushOrder(OrderTerms)` is the only place an `Order` is constructed or queued** — it derives `amountDue = _toTokenUnits(terms.offerAmountNormalized18AfterFees, wantAsset.decimals())` and stamps `queuedAt` at construction, so a half-initialized `Order` (placeholder zeros) never exists anywhere, and wire data can never smuggle an `amountDue` (the wire only carries terms).
