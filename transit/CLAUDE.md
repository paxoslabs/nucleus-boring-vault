...
- `_lzReceive(origin, guid, payload, …)` — decodes `OrderTerms`, **re-validates the route against `approvedRoutes`** (destEID = thisChainEID), wraps them in an `Order` and pushes (`_pushOrder` derives `amountDue` from the bridged normalized net + local `wantAsset.decimals()`, stamps `queuedAt`, and returns the populated copy), then emits `OrderBridgeReceived(uuid, origin.srcEid, guid, order)`. Sender authenticity enforced upstream by `OAppAuthReceiver.lzReceive` (`peers[srcEid]`). A revert here is safe — delivery is **unordered** (`nextNonce` returns 0), so the message stays retryable on the endpoint until the route is re-approved; nothing is consumed/lost (see Decisions Log 2026-06-06).
