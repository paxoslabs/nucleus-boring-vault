...
- `executePendingOrders(bytes32[] uuids, uint256[] amounts) requiresAuth` — EXECUTOR fulfills by `safeTransferFrom`-ing wantAsset from `wantAssetSource` directly to the receiver (KDD 26 — station custodies nothing), decrements `amountDue`, removes when zero (KDD 8 partial fills). **Derives the distinct want assets touched on-chain** (O(n²) dedup) and asserts `allowance(wantAssetSource, this) == 0` for each after the fills (else `ResidualApproval(token, remaining)`) — enforces no dangling approval (KDD 26). **Operational cost: the vault must approve EXACTLY the per-token batch total before each execute, or it reverts.** Reverts the batch on any failure with the offending `uuid` (`OrderNotFound(uuid)` / `AmountExceedsDue(uuid)`, KDD 19).
