...
- *2026-06-10*: **Distributor code (KDD 29).** Added `bytes distributorCode` to the signed `Quote` — an arbitrary referral tag so enterprises can log flow as theirs ("this flow was mine"; pure attribution, distinct from the integrator fee — no funds move). Mirrors `DistributorCodeDepositor`'s pattern: emitted as a `bytes indexed` field on `OrderSubmitted` (filter by topic `keccak256(code)`). Design calls: **signed into the quote** (backend authenticates the integrator, so attribution is backend-attested — a tampered code fails signature verification and changes the UUID) and **source-only** (not in `OrderTerms`, never bridged or stored — attribution is a source-chain fact; cross-chain joins by `uuid`). EIP-712: `bytes` hashes as `keccak256(contents)` in `_hashQuote`; `QUOTE_TYPEHASH` updated. Test note: solc 0.8.21 ICEs on `emit TransitStation.OrderSubmitted(...)` (qualified access to a NatSpec'd event — fixed in 0.8.22), so the test declares a local mirror event for `vm.expectEmit`.
