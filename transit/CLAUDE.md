...
8 tests — deploy script, single-chain submit→execute, cross-chain bridge attempt, decimal-scaling both
directions, sub-dust reverts, fee-dust partition). Testnet deployments (Sepolia 11155111, Robinhood 46630,
...
signer CAN still do: grief fees (`protocolFee` capped at `MAX_PROTOCOL_FEE_BPS`; `integratorFee` uncapped →
**open: add `MAX_INTEGRATOR_FEE_BPS`** — it can skim a legit user's deposit to an attacker
`integratorFeeReceiver`) and mis-route within the owner-approved set. The 1:1 assumption itself is an
...
Fee model: two fees (normalized 18-decimal units, like all quote amounts), both computed off-chain and signed into the quote. `protocolFeeNormalized18` (PXL's cut) → `protocolFeeRecipient`, capped at `MAX_PROTOCOL_FEE_BPS`. `integratorFeeNormalized18` (the frontend's own UniswapX-style fee) → the per-quote `integratorFeeReceiver`, **uncapped on-chain** (open item: cap it — under the derive-on-chain model it's no longer a *vault* drain, but a compromised signer can still skim a legit user's deposit). The net is pulled to `offerReceiver` (in practice a BoringVault) in token units, and the **want owed is derived from the collected net** (re-normalized, then truncated into want units on the destination) rather than taken from the quote.
...
- `OrderSubmitted(uuid, route, terms, user)` — fully collected + dispatched (local or bridged). Carries `OrderTerms` — identical/honest on both paths (no placeholder zeros); the queued state is in `OrderReceived` for same-chain orders and only exists on the destination for bridged ones.
...
5. **Fee cap — REWORKED (2026-06-10).** `protocolFee` capped at `MAX_PROTOCOL_FEE_BPS = 50` (0.5%, hardcoded immutable); `integratorFee` still **uncapped on-chain → OPEN: add `MAX_INTEGRATOR_FEE_BPS`**. The old "users see `amountDue` before submitting" rationale is GONE (`amountDue` is no longer in the quote). With the rate derived on-chain (KDD 27), `integratorFee` is the remaining signer-skim vector — bounded **user-fund** risk (skim a legit user's deposit to an attacker `integratorFeeReceiver`), no longer a vault drain. Compromised `quoteSigner` rotatable via `setQuoteSigner`; `pause()` freezes submit + execute.
...
- `TransitStation.sol` feature-complete through the normalized-amounts redesign (KDD 27); Foundry suite green (8 tests, incl. decimal scaling + sub-dust + fee-dust cases).
...
- **Open / next:**
  - **`MAX_INTEGRATOR_FEE_BPS`** — agreed worth doing, not yet implemented (the remaining signer-skim vector; bounded user-fund risk, not a vault drain).
